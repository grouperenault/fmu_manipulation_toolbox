...
!!! info "What is an FMU?"
    A Functional Mock-up Unit (FMU) is a standardized format for model exchange and co-simulation defined by the [FMI Standard](https://fmi-standard.org/){:target="_blank"}. FMUs are widely used in automotive, aerospace, and industrial automation.
...
??? success "Installation successful!"
    ...
    You should see the help message displaying all available commands. If you encounter issues, check our [Troubleshooting Guide](help/troubleshooting.md).
...
=== "Command Line"
    ...
    ```bash
    fmutool -input my_module.fmu -summary
    ```
...
=== "Python API"
    ...
    print(f"Total Variables: {len(fmu.variables)}")
    print(f"Parameters: {fmu.count_parameters()}")
    ```
...
-   [:octicons-terminal-24: CLI Guide](user-guide/cli-usage.md)
    
    Master command-line operations

-   [:octicons-code-24: Python API](user-guide/python-api.md)
    
    Automate with Python scripts

-   [:octicons-apps-24: GUI Guide](user-guide/gui-usage.md)
    
    Use the graphical interface

-   [:octicons-light-bulb-24: Use Cases](examples/common-use-cases.md)
...
!!! success "Ready to Go!"
    ...
    Need help? Check our [FAQ](help/faq.md) or [report an issue](https://github.com/grouperenault/fmu_manipulation_toolbox/issues){:target="_blank"}.
