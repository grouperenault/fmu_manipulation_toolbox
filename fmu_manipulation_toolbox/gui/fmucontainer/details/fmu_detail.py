...
class FMUDetailWidget(QWidget):
    ...
    def _load_from_node(self, node):
        """(Re)populate the tables from *node*'s current data.

        Unlike :meth:`set_node`, this does NOT call :meth:`sync_to_node`
        first, so it is safe to call after a *programmatic* mutation of the
        node's `user_start_values` / `user_exposed_inputs` / `user_exposed_outputs`
        (e.g. from the MCP/AI bridge) to keep the table widgets in sync and
        avoid a later `sync_to_node()` overwriting the fresh data with stale
        table content.
        """
