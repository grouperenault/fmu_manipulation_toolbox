...
* an FMU: see [`container.c`](container.c) which implements the logic, [`fmi2.c`](fmi2.c) and [`fmi3.c`](fmi3.c) which implement the FMI API
...
* the container configuration (inputs, outputs, time step, miscellenious options)
...
Values are conveyed from an FMU to other using buffers (see `container_t`). So that, connection one-to-many
are possible and container can expose conveyed date through local ports.
